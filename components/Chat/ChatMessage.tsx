...
import ChartBar from '../Chatbar/components/ChartBar';
import ChartPie from '../Chatbar/components/ChartPie';
...
export const ChatMessage: FC<Props> = memo(
  ({ message, messageIndex, onEdit }) => {
    ...
    const { messages }: any = selectedConversation || {};
    ...
    const markdownTableToCSV = (markdown: string): string => {
      ...
      // 找到表格的起始和结束位置
      const tableStart = markdown.indexOf('|') !== -1 ? markdown.indexOf('|') : 0;
      ...
      // 分割表头和数据行
      const headerRow = rows[0].substring(1, rows[0].length - 1).split('|');
      const dataRows = rows.slice(1, -1).map(row => row.substring(1, row.length - 1).split('|'));
    
      // 去除表头和数据行中的空格并去除空白行
      const cleanedHeader = headerRow.map(col => col.trim());
      const cleanedDataRows = dataRows.map(row => row.map(col => col.trim())).filter(row => row.length > 0);
    
      // 构建CSV头部
      const csvHeader = cleanedHeader.join(',');
    
      // 构建CSV行
      const csvRows = cleanedDataRows.map(row => row.join(','));
    ...
    return (
      ...
      >
        {/* m-auto */}
        <div className="relative mar-demo flex p-4 text-base md:max-w-2xl md:gap-6 md:py-6 lg:max-w-2xl lg:px-0 xl:max-w-3xl">
